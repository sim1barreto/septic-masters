...
import { Phone, Star, ArrowRight, Shield, Clock, CheckCircle, ChevronDown } from 'lucide-react';
...
const DEFAULTS = {
  badgeText: '24/7 Emergency Service Available',
  headline: "Central Texas' #1 Septic Service Experts",
  subheadline:
    'Professional septic pumping, inspections, and repairs. Fast, reliable, and done right the first time — backed by 15+ years of trusted service.',
  phone: '(555) 123-4567',
  ...
  return (
    <>
      ...
      <span
        ...
        style={{ backgroundImage: 'linear-gradient(135deg, #25A55F 0%, #4FD4A4 100%)' }}
      >
        {accent}
      </span>
...
const statIcons = [Clock, Star, Shield];

export function HeroBlock({ attrs }: BlockProps) {
  const badgeText = (attrs.badgeText as string) ?? DEFAULTS.badgeText;
  const headline = (attrs.headline as string) ?? DEFAULTS.headline;
  const subheadline = (attrs.subheadline as string) ?? DEFAULTS.subheadline;
  const phone = (attrs.phone as string) ?? DEFAULTS.phone;
  const ctaText = (attrs.ctaText as string) ?? DEFAULTS.ctaText;
  const ctaUrl = (attrs.ctaUrl as string) ?? DEFAULTS.ctaUrl;
  const heroImageUrl = (attrs.heroImageUrl as string) ?? DEFAULTS.heroImageUrl;
  const trustPoints = (attrs.trustPoints as string[]) ?? DEFAULTS.trustPoints;
  const stats = (attrs.stats as Stat[]) ?? DEFAULTS.stats;
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#BE2026]/20 border border-[#BE2026]/40 backdrop-blur-sm">
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#BE2026] opacity-75" />
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#BE2026]" />
              </span>
              <span className="text-red-300 text-sm font-bold">{badgeText}</span>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {['bg-green-400', 'bg-blue-400', 'bg-orange-400', 'bg-purple-400'].map(
                  (color, i) => (
                    <div
                      key={i}
                      className={`w-9 h-9 rounded-full ${color} border-2 border-[#0B2545] flex items-center justify-center text-white text-xs font-bold`}
                    >
                      {['JD', 'SM', 'RK', 'AL'][i]}
                    </div>
                  )
              ...
              <div>
                ...
                <div className="text-white/70 text-xs font-medium">500+ happy customers</div>
              </div>
            </div>
            <div className="h-8 w-px bg-white/20" />
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-[#25A55F]" />
              <span className="text-white/70 text-sm font-medium">Licensed & Insured</span>
            </div>
          </motion.div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div
                ...
                style={{ backgroundColor: '#25A55F25', border: '1px solid #25A55F50' }}
              >
      ...
      >
        <span className="text-white/40 text-xs tracking-widest uppercase font-semibold">Scroll</span>
